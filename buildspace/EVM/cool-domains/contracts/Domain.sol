...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("Ass Name Service", "ANS") {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Ass name service", "image": "data:image/svg+xml;base64,',
    ...
    function getAllNames() public view returns (string[] memory) {
        ...
        for (uint256 i = 0; i < _tokenIds.current(); i++) {
            allNames[i] = names[i];
            console.log("Names for token %d is %s", i, allNames[i]);
        }
